...
class SalesEmployeeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Employee
        fields = ['id','name','notes','truck_num','phonenumber','phonenumber2','longitude','latitude','address']
...
class DepositeSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if instance.registry != registry:
            raise serializers.ValidationError({"خطأ":"لا يسمح لك بتعديل هذا الصندوق"})
        
        difference = validated_data['total'] - instance.total
        if registry.total - abs(difference) < 0:
            raise serializers.ValidationError({"خطأ":"رصيد الخزنة لا يمكن أن يكون أقل من صفر"})
...
class ProductsOutputSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Output_Products
        fields = ['id', 'product_id','name', 'num_per_item', 'sale_price', 'num_item', 'total_price', 'discount', 'output','points']
...
class OutputSerializer2(serializers.ModelSerializer):
    ...
    class Meta:
        model = Output
        exclude = ['employee','location','client']
